...
func Test(t *testing.T) {
	...
	t.Run("attribute expressions can return errors", func(t *testing.T) {
		...
		t.Run("the errors are templ errors", func(t *testing.T) {
			...
			if templateErr.FileName != `test/templ/test-attribute-errors/template.templ` {
				t.Errorf("expected error in `test/templ/test-attribute-errors/template.templ`, but got %v", templateErr.FileName)
			}
